...
/**
 * @file script.js
 * @description A professional, class-based implementation of Checkers (Draughts).
 * This file consolidates the Game, UI, AI, and Audio controllers to ensure
 * compatibility with local file execution while maintaining modular architecture.
 *
 * Key Features:
 * - MVC-like architecture separating Logic, UI, and AI and Audio.
 * - Web Audio API integration for synthesized sound effects.
 * - Canvas API implementation for visual particle effects.
 * - Robust move validation and game state management.
 *
 ...
 *
 * @param {number} piece - The numeric representation of the piece.
 ...
 *
 * @param {number} piece - The numeric representation of the piece.
 * @returns {Array<Object>} Array of direction objects {rowOffset, colOffset}.
 */
 ...
 * Handles browser compatibility and audio context state (suspended/running).
 */
class AudioController {
     ...
     *
     * @param {string} type - The oscillator type (e.g., 'sine', 'square', 'triangle').
     ...
     * @param {number} [freqEnd] - Optional ending frequency for frequency ramping effects.
     */
     ...
     *
     * @param {string} type - The sound type ('move', 'capture', 'king', 'win').
     */
    playSound(type) {
        if (!this.audioCtx) return;
        ...
        if (type === 'move') {
            this.playTone(AUDIO_CONFIG.MOVE.type, AUDIO_CONFIG.MOVE.freq, AUDIO_CONFIG.MOVE.duration, AUDIO_CONFIG.MOVE.volStart, AUDIO_CONFIG.MOVE.volEnd, AUDIO_CONFIG.MOVE.freqEnd);
        } else if (type === 'capture') {
            this.playTone(AUDIO_CONFIG.CAPTURE.type, AUDIO_CONFIG.CAPTURE.freq, AUDIO_CONFIG.CAPTURE.duration, AUDIO_CONFIG.CAPTURE.volStart, AUDIO_CONFIG.CAPTURE.volEnd, AUDIO_CONFIG.CAPTURE.freqEnd);
        } else if (type === 'king') {
...
class AIController {
     ...
     * @returns {Object|null} The selected move object or null if no moves.
     */
    makeMove() {
     ...
     * @param {Array<Object>} moves - The list of available moves.
     * @returns {Array<Object>} The subset of best moves.
     */
     ...
     * @param {Object} move - The move to simulate.
     * @returns {boolean} True if a chain jump is possible after this move.
     */
     ...
     * @returns {Array<Object>} Valid moves for the piece.
     */
...
class UIController {
    /**
     * Initializes the UI controller and sets up the DOM structure.
     * @param {Game} game - Reference to the main Game instance.
     */
    constructor(game) {
        this.game = game;
    ...
    initLayout() {
        ...
        this.controlsGroup = createElement('div', this.gameLayout, {
            cssText: "display: flex; flex-direction: column; gap: 2px; position: absolute; right: 100%; top: 40px; margin-right: 35px; width: max-content;"
        });
    ...
    createScoreboard() {
        ...
        createElement('div', scoreGroup, { text: LABELS.SCOREBOARD, cssText: `font-weight: bold; font-size: 14px; text-decoration: none; border-bottom: 1px solid ${THEME.SCORE_TITLE_BORDER}; padding-bottom: 5px; margin-bottom: 5px; letter-spacing: 1px;` });
        ...
        const resetScoreBtn = createElement('button', scoreGroup, { text: LABELS.BTN_RESET_SCORE, cssText: `margin-top: 15px; padding: 8px 12px; background: ${THEME.RESET_SCORE_BTN_BG}; color: ${THEME.RESET_SCORE_BTN_TEXT}; border: 1px solid ${THEME.RESET_SCORE_BTN_BORDER}; cursor: pointer; font-family: inherit; font-weight: bold; font-size: 12px; border-radius: 2px; transition: all 0.2s;` });
    ...
    createCapturedContainer() {
        const capturedContainer = createElement('div', this.gameLayout, { cssText: "display: flex; flex-direction: column; gap: 5px; position: absolute; right: 100%; bottom: 25px; margin-right: 10px;" });
    ...
    addCapturedPiece(piece) {
        const classes = [CSS_CLASSES.PIECE, isRedPiece(piece) ? CSS_CLASSES.RED : CSS_CLASSES.BLACK];
        if (isKing(piece)) classes.push(CSS_CLASSES.KING);
        const pieceElement = createElement('div', null, { className: classes.join(' '), cssText: "width: 30px; height: 30px; margin: 0; font-size: 14px; cursor: default; box-shadow: inset 0 0 0 2px rgba(0,0,0,0.2), inset 0 0 0 4px rgba(255,255,255,0.1);" });
     ...
     */
    initConfetti() {
        this.confettiCanvas = createElement('canvas', document.body, { cssText: "position: fixed; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 9999;" });
...
class Game {
    ...
    updateStatus() {
        ...
        if (this.chainJumpInProgress) {
            if (this.isSinglePlayer) text = this.isRedTurn ? LABELS.STATUS_STILL_YOUR : LABELS.STATUS_STILL_MY;
            else text = this.isRedTurn ? LABELS.STATUS_STILL_RED : LABELS.STATUS_STILL_BLACK;
        } else {
            if (this.isSinglePlayer) text = this.isRedTurn ? LABELS.STATUS_YOUR : LABELS.STATUS_MY;
            else text = this.isRedTurn ? LABELS.STATUS_RED : LABELS.STATUS_BLACK;
        }
    ...
    handleSquareClick(event) {
        try {
            ...
            if (this.chainJumpInProgress) {
                ...
                if (this.board[row][col] !== PIECES.EMPTY && this.isRedTurn === isRedPiece(this.board[row][col])) {
     ...
     */
    isValidMove(row, col) {
        if (this.board[row][col] !== PIECES.EMPTY || (row + col) % 2 === 0) return false;

        const piece = this.board[this.selectedRow][this.selectedCol];
        const king = isKing(piece);
        const rowDiff = Math.abs(row - this.selectedRow);
        const colDiff = Math.abs(col - this.selectedCol);

        if (!king) {
            if (this.isRedTurn && row > this.selectedRow) return false;
            if (!this.isRedTurn && row < this.selectedRow) return false;
        }

        if (this.chainJumpInProgress) {
            return rowDiff === 2 && colDiff === 2 && this.isJump(row, col);
        }

        if (rowDiff === 1 && colDiff === 1) {
            if (king) return true;
            return this.isRedTurn ? row < this.selectedRow : row > this.selectedRow;
        } else if (rowDiff === 2 && colDiff === 2) {
            return this.isJump(row, col);
        }
        return false;
    }
    ...
    checkWinner() {
        ...
        if (winner) {
            ...
            this.ui.winnerMessage.textContent = (this.isSinglePlayer && winner === LABELS.RED) ? LABELS.WIN_YOU : (this.isSinglePlayer ? LABELS.WIN_JOHN : `${winner} wins!`);
    ...
    toggleSinglePlayer() {
        ...
        this.ui.profilePic.style.borderColor = this.isSinglePlayer ? THEME.PROFILE_BORDER_ACTIVE : THEME.PROFILE_BORDER_INACTIVE;
     ...
     * @param {number} row - Row index.
     * @param {number} col - Column index.
     * @returns {Array<Object>} List of valid move objects {fromRow, fromCol, toRow, toCol}.
     */
    getMovesForPiece(row, col) {
        const moves = [];
        const deltas = [
        ...
        // Temporarily set selection to check validity
        const prevSelR = this.selectedRow;
        const prevSelC = this.selectedCol;
        this.selectedRow = row;
        this.selectedCol = col;
        ...
        for (const delta of deltas) {
            ...
            if (isValidPos(targetRow, targetCol) && this.isValidMove(targetRow, targetCol)) {
                moves.push({fromRow: row, fromCol: col, toRow: targetRow, toCol: targetCol});
            }
        }
        this.selectedRow = prevSelR;
        this.selectedCol = prevSelC;
        return moves;
    }
}
